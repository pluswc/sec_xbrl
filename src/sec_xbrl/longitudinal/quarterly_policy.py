        ...
        *,
        ...
        if not run_version or "/" in run_version or "\\" in run_version:
            raise QuarterlyPeriodPolicyError(
                "quarterly policy run_version must be a non-path identifier"
            )
        root = Path(output_root)
        target = root / run_version
        ...
        manifest = {
            "contract_version": QUARTERLY_POLICY_VERSION,
            ...
            "validation": {
                "VERIFIED_C3_M1_LINKAGE": "SUCCESS",
                "RAW_LINEAGE": "SUCCESS",
                "DETERMINISTIC_OUTPUT": "SUCCESS",
        ...
        if target.exists():
            ...
            if (
                ...
                or json.loads(existing.read_text(encoding="utf-8")) != manifest
            ):
                raise QuarterlyPeriodPolicyError(
                    "quarterly policy run_version already exists with different content"
                )
            return QuarterlyPolicyPublication(
                target, existing, manifest["upstream_layer2_run_fingerprint"], counts
            )
        ...
        try:
            ...
            raise
        return QuarterlyPolicyPublication(
            target, target / self.manifest_name, manifest["upstream_layer2_run_fingerprint"], counts
        )
...
class QuarterlyPolicyPublicationReader:
    ...
    def load(self, run_root: Path, *, upstream: VerifiedLayer2Publication) -> QuarterlyPolicyResult:
        root = Path(run_root)
        manifest_path = root / QuarterlyPolicyPublisher.manifest_name
        if (
            not root.is_dir()
            or root.is_symlink()
            ...
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion release is missing or unsafe"
            )
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion manifest is invalid"
            ) from exc
        expected = {
            "contract_version",
            "run_version",
        ...
        if (
            ...
            or manifest.get("contract_version") != QUARTERLY_POLICY_VERSION
        ):
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion manifest has unsupported contract"
            )
        if manifest.get("upstream_layer2_run_fingerprint") != upstream.identity.get(
            "layer2_run_fingerprint"
        ):
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion does not match verified C3-M1 publication"
            )
        if manifest.get("upstream_layer2_manifest_sha256") != upstream.identity.get(
            "layer2_manifest_sha256"
        ):
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion does not match verified C3-M1 manifest"
            )
        if set(manifest.get("output_counts", ())) != set(_DATASETS) or set(
            manifest.get("output_content_sha256", ())
        ) != set(_DATASETS):
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion manifest datasets are incomplete"
            )
        expected_files = {
            QuarterlyPolicyPublisher.manifest_name,
            *(f"{name}.jsonl" for name in _DATASETS),
        }
        actual_files = {
        ...
        if actual_files != expected_files or any(
            ...
            raise QuarterlyPeriodPolicyError(
                "quarterly policy companion layout is incomplete or unexpected"
            )
        ...
        for name in _DATASETS:
            ...
            try:
                ...
                raise QuarterlyPeriodPolicyError(
                    "quarterly policy companion dataset is invalid"
                ) from exc
            if (
                any(not isinstance(row, dict) for row in values)
                or _hash_rows(values) != manifest["output_content_sha256"].get(name)
                or len(values) != manifest["output_counts"].get(name)
            ):
                raise QuarterlyPeriodPolicyError(
                    "quarterly policy companion content verification failed"
                )
            rows[name] = tuple(dict(row) for row in values)
        return QuarterlyPolicyResult(
            rows["quarterly_q4_candidate"],
            rows["quarterly_q4_exclusion"],
            rows["predecessor_period_linkage"],
        )
    ...
    def materialize(
        ...
        *,
        ...
        if not isinstance(publication, VerifiedLayer2Publication) or not isinstance(
            ...
            raise QuarterlyPeriodPolicyError(
                "C3-M2 requires verified C3-M1 publication and CorpusRelease"
            )
        if publication.identity["layer2_run_fingerprint"] != release.layer2_run.fingerprint:
            raise QuarterlyPeriodPolicyError(
                "C3-M1 publication does not match supplied CorpusRelease declaration"
            )
...
def _declarations(
    ...
    for row in rows:
        if (
            ...
            or not row.company_canonical_concept_id
            or not row.declaration_id
        ):
            raise QuarterlyPeriodPolicyError(
...
def _attach_raw_semantics(
    facts: Iterable[Mapping[str, Any]], release: CorpusRelease
) -> tuple[dict[str, Any], ...]:
    raw = {
        (str(row.get("filing_id")), str(row.get("fact_id"))): row for row in release.records("fact")
    }
    concepts = {
        (str(row.get("filing_id")), str(row.get("raw_concept_id"))): row
        for row in release.records("concept")
    }
    units = {
        (str(row.get("filing_id")), str(row.get("unit_id"))): row for row in release.records("unit")
    }
    ...
    for source in facts:
        ...
        if raw_fact is None:
            raise QuarterlyPeriodPolicyError(
                "C3-M1 selected raw Fact is absent from verified CorpusRelease"
            )
...
def _q4_compatibility(fy: Mapping[str, Any], ytd: Mapping[str, Any]) -> str | None:
    ...
    fy_bounds, ytd_bounds = (
        fy.get("actual_period_boundaries") or (),
        ytd.get("actual_period_boundaries") or (),
    )
    if (
        len(fy_bounds) < 2
        or len(ytd_bounds) < 2
        or not fy_bounds[0]
        or fy_bounds[0] != ytd_bounds[0]
    ):
...
def _q4_candidate(
    ...
    return {
        "quarterly_policy_candidate_id": _id("quarterly-q4", identity),
        "cik": fy["cik"],
        "view": "AS_FILED",
        ...
        "formula": "FY - YTD_9M",
        "input_analytical_fact_ids": source_ids,
        "input_source_fact_ids": (fy["selected_fact_id"], ytd["selected_fact_id"]),
...
def _predecessors(facts: tuple[dict[str, Any], ...]) -> list[dict[str, Any]]:
    ...
    for _, rows in sorted(groups.items(), key=repr):
        ordered = sorted(
            rows, key=lambda item: (str(item["period_key"]), str(item["analytical_fact_id"]))
        )
        for position, current in enumerate(ordered):
            ...
            identity = (
                current["analytical_fact_id"],
                None if predecessor is None else predecessor["analytical_fact_id"],
            )
            output.append(
                {
                    "predecessor_period_linkage_id": _id("predecessor-period", identity),
                    "cik": current["cik"],
                    "analytical_fact_id": current["analytical_fact_id"],
                    "predecessor_analytical_fact_id": None
                    if predecessor is None
                    else predecessor["analytical_fact_id"],
                    "period_class": current["period_class"],
                    "company_canonical_concept_id": current["company_canonical_concept_id"],
                    "company_canonical_dimension_key": current.get(
                        "company_canonical_dimension_key"
                    ),
                    "basis_version": current.get("basis_version"),
                    "unit_semantics": current.get("unit_semantics"),
                    "link_status": "ELIGIBLE" if predecessor else "UNAVAILABLE",

...
fixed reviewed US-GAAP local-name allowlist, an empty dimension scope, and a
retained PRE relationship in a controlled primary consolidated Income /
Operations or Cash Flows role.  Both FY and YTD source Facts must independently
meet this evidence test.

A declaration identity contains normalized CIK, company canonical concept,
complete canonical dimension key, basis version, and unit semantics.  M2-v2
...
dimensioned, custom, EPS/share/ratio/instant Fact through a concept-only match.
